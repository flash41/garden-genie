...
function gridRefToMetres(
  ...
  const row = parseInt(clean[1] || '3') - 1;
...
function clampGridLocation(loc: string | undefined | null): string {
  if (!loc || typeof loc !== 'string') return null as any;
...
// ─── INNGEST PIPELINE FUNCTION ────────────────────────────────────────────────

export const pipelineFunction = inngest.createFunction(
  { id: 'design-pipeline', retries: 0, triggers: [{ event: 'pipeline/start' }] },
  async ({ event, step }) => {
    const {
      jobId,
      inviteCode,
    ...
    try {
      // a. download-image
      const { imageBase64, mimeType } = await step.run('download-image', async () => {
        ...
        return { imageBase64: base64, mimeType: originalMimeType };
      });

      // b. spatial-fingerprint
      const fingerprint = await step.run('spatial-fingerprint', async () => {
        const fp = await step1_spatialFingerprint(imageBase64, mimeType, orientation);
        if (hardinessZone) fp.hardinessZone = hardinessZone;
        return fp;
      });

      // c. control-points
      const { controlPoints, g2Grid } = await step.run('control-points', async () => {
        const cp = await step2b_extractControlPoints(imageBase64, mimeType, fingerprint);
        const grid = deriveG2Grid(cp);
        return { controlPoints: cp, g2Grid: grid };
      });

      // d. garden-design
      const creativityDescription = TRANSFORMATION_DESCRIPTIONS[creativityLevel] || TRANSFORMATION_DESCRIPTIONS[3];
      const designJSON = await step.run('garden-design', async () => {
        return await step2_gardenDesign(
          imageBase64,
          mimeType,
          fingerprint,
          style,
